...
//
//  Gated to the index capability's platform: the only consumer is
//  IndexStoreReader (itself `#if canImport(IndexStoreDB)`), and the `st_mode`
//  bit math below is Darwin-typed (`UInt16`), so excluding it on Linux both
//  removes dead code and avoids the `__mode_t` (UInt32) portability mismatch.
